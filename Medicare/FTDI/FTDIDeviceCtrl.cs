    ...
    public class FTDIDeviceCtrl
    {
        ...
        public static List<FTDIDevice> GetDeviceList()
        {
            ...
            try
            {
                ...
                {
                    Util.openPopupOk($"\tFailed to get number of devices");
                    return devices;
                }

                if (deviceCount == 0)
                {
                    Util.openPopupOk($"\tNo FTDI devices found.");
                    return devices;
                }
                ...
                if (ftStatus != FT_STATUS.FT_OK)
                {
                    ...
                    Util.openPopupOk($"\tFailed to get device list");
                    return devices;
                }
                ...
                foreach (var device in deviceList)
                {
                    if (device.Type == FTDI.FT_DEVICE.FT_DEVICE_232R) // FT232R 디바이스만 리스트에 추가
                    {
                        ...
                        //Util.openPopupOk($"\tFind {device.Description} ({device.SerialNumber}) - {comPortName}");
                    }
                }
            }
            catch (Exception ex)
            {
                Util.openPopupOk($"\tException : {ex.Message}");
                Util.openPopupOk($"\tFailed to activate GPIO. Please verify configuration and try again.");
            }
        ...
        public static bool DeviceOpen(OPEN_TYPE type, string deviceIdentifier)
        {
            ...
            {
                Util.openPopupOk($"\tValue cannot be parsed to UInt32");
                return result;
            ...
            {
                Util.openPopupOk($"\tException : {e.Message}");
                Util.openPopupOk($"\tFailed to activate GPIO. Please verify configuration and try again.");
                return result;
            }
        ...
        public static bool DeviceClose()
        {
            ...
            {
                Util.openPopupOk($"\tException : {e.Message}");
                Util.openPopupOk($"\tFailed to close the FTDI device. Please verify configuration and try again.");
                return result;
            }
            ...
            try
            {
                if (_ftdi.IsOpen)
                {
                    ...
                    if (mode == BIT_MODE.ON)
                    {
                        ...
                        if (ftStatus != FT_STATUS.FT_OK)
                        {
                            Util.openPopupOk($"\tFailed to turn on VCC. Please check the device and try again.");
                            return result;
                        }
            ...
            try
            {
                if (_ftdi.IsOpen)
                {
                    ...
                    else if (mode == BIT_MODE.OFF)
                    {
                        ...
                        if (ftStatus != FT_STATUS.FT_OK)
                        {
                            Util.openPopupOk($"\tFailed to deactivate GPIO.Please verify configuration and try again.");
                            return result;
                        }
        ...
        public static bool SetPowerAndGPIO(OPEN_TYPE type, string deviceIdentifier, BIT_MODE mode)
        {
            ...
            try
            {
                ...
                {
                    Util.openPopupOk($"\tFailed to device open. {ftOpenStatus.ToString()}.");
                    return result;
                }
                ...
                if (mode == BIT_MODE.POWER_ON)
                {
                    ...
                    //if (ftStatus != FT_STATUS.FT_OK)
                    //{
                    //    Util.openPopupOk("Failed to turn on VCC. Please check the device and try again.", typeof(FTDIDeviceManager).Name);
                    //    return result;
                    //}
        ...
        public static bool SetBitBang(OPEN_TYPE type, string deviceIdentifier, BIT_MODE mode)
        {
            ...
            try
            {
                ...
                if (mode == BIT_MODE.LEVEL_SHIFT_ON)
                {
                    ...
                    mask |= _maskLevelShiftOn; 
                    ftStatus = _ftdi.SetBitMode(mask, _maskBitBangMode); // CUBS Pin 상태를 High 로 변경
                    if (ftStatus != FT_STATUS.FT_OK)
                    {
                        Util.openPopupOk($"\tFailed to turn on Level shift. Please verify configuration and try again.");
                        return result;
                    }
                ...
                else if (mode == BIT_MODE.LEVEL_SHIFT_OFF)
                {
                    ...
                    if (ftStatus != FT_STATUS.FT_OK)
                    {
                        Util.openPopupOk($"\tFailed to turn off Level shift. Please check the device and try again.");
                        return result;
                    }
                ...
                {
                    FT_STATUS ftStatus = _ftdi.SetBitMode(0x0, _maskBitBangMode); // CUBS Pin 상태를 low 로 변경
                    if (ftStatus != FT_STATUS.FT_OK)
                    {
                        Util.openPopupOk($"\tFailed to turn off Level shift. Please check the device and try again.");
                        return result;
        ...
        private static bool readsetbits()
        {
            DeviceOpen(FDTI_Type.OPEN_TYPE.DESCRIPTION, "dut");
            FT_STATUS ftStatus = _ftdi.SetBitMode(0x00, _maskBitBangMode);
            Thread.Sleep(waittime);

            if (ftStatus != FT_STATUS.FT_OK)
            {
                Util.openPopupOk($"readsetbits() - set bit mode error.");
                DeviceClose();
                return false;
            }
        ...
        public static bool setOne(string serialno)
        {
            if (!readsetbits()) return false;
            //1번
            DeviceOpen(FDTI_Type.OPEN_TYPE.SERIALNUMBER, serialno);
            byte mask = 0x00;
            mask |= 0x44;

            FT_STATUS ftStatus = _ftdi.SetBitMode(mask, _maskBitBangMode);
            Thread.Sleep(waittime);
            
            if (ftStatus != FT_STATUS.FT_OK)
            {
                Util.openPopupOk($"setOneTwo() - Step One - 1: set bit mode error.");
                return false;
            }

            mask |= 0x22;
            ftStatus = _ftdi.SetBitMode(mask, _maskBitBangMode);
            
            
            if (ftStatus != FT_STATUS.FT_OK)
            {
                Util.openPopupOk($"setOneTwo() - Step One - 2 : set bit mode error.");
                DeviceClose();
                return false;
            }
            DeviceClose();
        ...
        public static bool setTwo(string serialno)
        {
            DeviceOpen(FDTI_Type.OPEN_TYPE.SERIALNUMBER, serialno);
            byte mask = 0x00;
            // 2번
            ...
            if (ftStatus != FT_STATUS.FT_OK)
            {
                Util.openPopupOk($"setOneTwo() - Step Tow : set bit mode error.");
                DeviceClose();
                return false;
            }

            //ftStatus = _ftdi.GetPinStates(ref mask);
            //Thread.Sleep(waittime);

            //if (ftStatus != FT_STATUS.FT_OK)
            //{
            //    Util.openPopupOk($"Get Pin States Error.");
            //    DeviceClose();
            //    return false;
            //}
        ...
        public static bool setFour(string serialno)
        {
            //if (!readsetbits()) return false;
        ...
        public static bool setFive(string serialno)
        {
            DeviceOpen(FDTI_Type.OPEN_TYPE.SERIALNUMBER, serialno);

            byte mask = 0x00;
            mask = (byte) (mask | 0x20 | 0x40 | 0x80);
