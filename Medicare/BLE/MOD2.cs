...
namespace Medicare.BLE
{
    public partial class MOD2 : Form
    {
        ...
        {
            mod2CondLabel.BackColor = Color.FromArgb(0, 126, 222);
            mod2CondLabel.ForeColor = Color.White;

            mod2SpecLabel.BackColor = Color.FromArgb(0, 126, 222);
            mod2SpecLabel.ForeColor = Color.White;
        ...
        private void load()
        {
            channelTextBox.Text = CTPMain.btConfigFile["mo2"]["channel"].ToString();
            packetNumTextBox.Text = CTPMain.btConfigFile["mo2"]["npackets"].ToString();
            df1avgUpperTextBox.Text = CTPMain.btConfigFile["mo2"]["df1avgupper"].ToString();
            df1avgLowerTextBox.Text = CTPMain.btConfigFile["mo2"]["df1avglower"].ToString();
            df2maxTextBox.Text = CTPMain.btConfigFile["mo2"]["df2max"].ToString();
            df2avgdf1avgTextBox.Text = CTPMain.btConfigFile["mo2"]["df2avgdf1avg"].ToString();
        }
        public static void modCharacteristicsCommands(out List<string> list)
        {
            ...
            // 1
            cmdList.Add("CNT,0");
            cmdList.Add(String.Format("CMD,CONFigure:BT:LE:TCase:MOD2M:CHANnel {0}", channel));
            cmdList.Add("CMD,CONFigure:BT:LE:TCase:MOD2M:DLENgth 37");
            cmdList.Add(string.Format("CMD,CONFigure:BT:LE:TCase:MOD2M:NPACkets {0}", CTPMain.packetNum[(int)eBLECaseIdx.MOD]));
            cmdList.Add(string.Format("CMD,CONFigure:BT:LE:TCase:MOD2M:LIMit:DF1_AVG_UP {0}", df1avgupper));
            cmdList.Add(string.Format("CMD,CONFigure:BT:LE:TCase:MOD2M:LIMit:DF1_AVG_LOW {0}", df1avglower));
            cmdList.Add(string.Format("CMD,CONFigure:BT:LE:TCase:MOD2M:LIMit:NUM_DF2_L {0}", df2max));
            cmdList.Add(string.Format("CMD,CONFigure:BT:LE:TCase:MOD2M:LIMit:DF2_DF1_LOW {0}", df2avgdf1avg));
            cmdList.Add("CMD,READ:BT:LE:TCase:MOD2M?");
            cmdList.Add("CMD,READ:BT:LE:TCase:MOD2M:VERDict?");
            list = cmdList;
        }
        public static string getMODReport(float time)
        {
            ...
            try
            {
                report = string.Format("==== Modulation Characteristics ====\n" +
                                       ...
                                       "   \tdf2/df1 >= {4}\n" +
                                       ":::Results:::\n"
                    , CTPMain.packetNum[(int)eBLECaseIdx.MOD2]
                    , df1avglower
                    , df1avgupper
                    , lower_df2_rate
                    , lower_df2_df1);
                ...
                var channel = CTPMain.channel[(int)eBLECaseIdx.MOD2].Split('-');
            ...
            {
                CTPMain.wnd.setTextPgrBar("MOD2 Error", Color.Red);
        ...
        private void save()
        {
            CTPMain.channel[(int)eBLECaseIdx.MOD2] = channelTextBox.Text;
            CTPMain.packetNum[(int)eBLECaseIdx.MOD2] = packetNumTextBox.Text;
            ...
            CTPMain.btConfigFile["mo2"]["channel"] = CTPMain.channel[(int)eBLECaseIdx.MOD2];
            CTPMain.btConfigFile["mo2"]["npackets"] = CTPMain.packetNum[(int)eBLECaseIdx.MOD2];
            CTPMain.btConfigFile["mo2"]["df1avgupper"] = df1avgupper;
            CTPMain.btConfigFile["mo2"]["df1avglower"] = df1avglower;
            CTPMain.btConfigFile["mo2"]["df2max"] = df2max;
            CTPMain.btConfigFile["mo2"]["df2avgdf1avg"] = df2avgdf1avg;
        ...
        private bool addRows()
        {
            ...
            int len = CTPMain.startIndex + (int)eTestItem.MODTIME_2M - (int)eTestItem.df1_avg_CH0_2M;
            ...
            for (int r = CTPMain.startIndex; r < len; r++)
            {
                ...
                int row = (r - CTPMain.startIndex) + (int)eTestItem.df1_avg_CH0_2M;

                if (row < (int)eTestItem.df2_max_CH0_2M)
                    CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.MEAS].Value = df1_avg[freIdx];
                else if ((int)eTestItem.df2_max_CH0_2M <= row && row <= (int)eTestItem.df2_max_CH39_2M)
            ...
            CTPMain.wnd.mainBLEGrid.Rows[len].Cells[(int)eCol.TST].Value = CTPMain.testTime[(int)eBLECaseIdx.MOD2];
        ...
        public async Task<bool> runMOD(List<string> cmdList)
        {
            try
            {
                ...
                CTPMain.testTime[(int)eBLECaseIdx.MOD2] = (float)(DateTime.Now - startTime).TotalSeconds;

                bool isPass = addRows();
                //bool isPass = true;
                //for (int r = (int)eTestItem.df1_avg_CH0_2M; r < (int)eTestItem.MODTIME_2M; r++)
                //{
                //    //await Task.Delay(CTPMain.waitTime);
                //    int freIdx = (r - (int)eTestItem.df1_avg_CH0_2M) % 3;
                //    CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.FREQ].Value = CTPMain.chFreq[freIdx];

                //    if (r <= (int)eTestItem.df2_max_CH0_2M)
                //        CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.MEAS].Value = df1_avg[freIdx];
                //    else if ((int)eTestItem.df2_max_CH0_2M < r && r <= (int)eTestItem.df2_max_CH39_2M)
                ...
                //    CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.UPPER].Value = df1avgupper;
                //}
                //CTPMain.wnd.mainBLEGrid.Rows[(int)eTestItem.MODTIME_2M].Cells[(int)eCol.TST].Value = CTPMain.testTime[(int)eBLECaseIdx.MOD2];
                CTPMain.wnd.mainBLEGrid.RefreshEdit();
                CTPMain.wnd.pgrBar.Value = CTPMain.prgCount += 10;
                if (!checkError(CTPMain.selectedRFPort))
                    return false;
                string report = getMODReport(CTPMain.testTime[(int)eBLECaseIdx.MOD2]);
