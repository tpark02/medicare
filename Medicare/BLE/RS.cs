...
namespace Medicare.BLE
{
    public partial class RS : Form
    {
        ...
        public void load()
        {
            channelTextBox.Text = CTPMain.btConfigFile["rs"]["channel"].ToString();
            packetNumTextBox.Text = CTPMain.btConfigFile["rs"]["npackets"].ToString();
            dirtyComboBox.SelectedIndex = Int32.Parse(CTPMain.btConfigFile["rs"]["dirty"].ToString());
            rsLevelTextBox.Text = CTPMain.btConfigFile["rs"]["rxlevel"].ToString();
            perTextBox.Text = CTPMain.btConfigFile["rs"]["per"].ToString();
        }

        public static void rsCommands(out List<string> list)
        {
            List<string> cmdList = new List<string>();
            var channel = CTPMain.channel[(int)eBLECaseIdx.RS];
            ...
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:SENS:CHANnel {0}", channel)); //채널 0
            cmdList.Add("CMD,CONF:BT:LE:TC:SENS:DLENgth 37");
            cmdList.Add("CMD,CONF:BT:LE:TC:SENS:PATTern 0");
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:SENS:NPACkets {0}", CTPMain.packetNum[(int)eBLECaseIdx.RS]));
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:SENS:TXPOWer {0}", rs_level));
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:SENS:DIRTY {0}", rs_dirty));
        ...
        {
            rsCondLabel.BackColor = Color.FromArgb(0, 126, 222);
            rsCondLabel.ForeColor = Color.White;

            rsSpecLabel.BackColor = Color.FromArgb(0, 126, 222);
            rsSpecLabel.ForeColor = Color.White;
        ...
        public static string getRSReport(float time)
        {
            ...
            try
            {
                report = string.Format("==== Receiver sensitivity, 1 M ====\n" +
                                       ...
                                       "       PER  <  {3} %\n" +
                                       ":::Results:::\n",
                    CTPMain.packetNum[(int)eBLECaseIdx.RS],
                    rs_level,
                    rs_dirty,
                    rs_per);
                ...
                var channel = CTPMain.channel[(int)eBLECaseIdx.RS].Split('-');
            ...
            {
                CTPMain.wnd.setTextPgrBar("RS Error", Color.Red);
        ...
        private void save()
        {
            CTPMain.channel[(int)eBLECaseIdx.RS] = channelTextBox.Text;
            CTPMain.packetNum[(int)eBLECaseIdx.RS] = packetNumTextBox.Text;
            rs_dirty = dirtyComboBox.SelectedIndex.ToString();
            rs_level = rsLevelTextBox.Text;
            rs_per = perTextBox.Text;

            CTPMain.btConfigFile["rs"]["channel"] = channelTextBox.Text;
            CTPMain.btConfigFile["rs"]["npackets"] = packetNumTextBox.Text;
            CTPMain.btConfigFile["rs"]["dirty"] = dirtyComboBox.SelectedIndex.ToString();
            CTPMain.btConfigFile["rs"]["rxlevel"] = rsLevelTextBox.Text;
            CTPMain.btConfigFile["rs"]["per"] = perTextBox.Text;
            ...
            try
            {
                ...
                foreach (var s in cmdList)
                {
                    ...
                    else if (l[0].Equals("DUT") && cmd.Equals(Util.hex2binary("0xC0") + Util.hex2binary("0x00")))
                    {
                        ...
                        //int totalCnt = Int32.Parse(CTPMain.packetNum[(int)eBLECaseIdx.RS]);

                        //packetRcvPer = (double)pktCnt / totalCnt;
                        //packetRcvPer *= 100.0;

                        //buf.Add(CTPMain.packetNum[(int)eBLECaseIdx.RS]);
                ...
                CTPMain.testTime[(int)eBLECaseIdx.RS] = (float)(DateTime.Now - startTime).TotalSeconds;

                bool isPass = addRows();
                //bool isPass = true;
                //for (int r = (int)eTestItem.RV_PER_CH0; r <= (int)eTestItem.RV_PER_CH39; r++)
                //{
                //    //await Task.Delay(CTPMain.waitTime);
                ...
                //CTPMain.wnd.mainBLEGrid.Rows[(int)eTestItem.RSTIME].Cells[(int)eCol.TST].Value =
                //    CTPMain.testTime[(int)eBLECaseIdx.RS];

                CTPMain.wnd.mainBLEGrid.RefreshEdit();
                CTPMain.wnd.pgrBar.Value = CTPMain.prgCount += 4;
                if (!checkError(CTPMain.selectedRFPort))
                    return false;
                string report = getRSReport(CTPMain.testTime[(int)eBLECaseIdx.RS]);
        ...
        private bool addRows()
        {
            ...
            int len = CTPMain.startIndex + (int)eTestItem.RSTIME - (int)eTestItem.RV_PER_CH0;

            for (int r = CTPMain.startIndex; r < len; r++)
            {
                int freIdx = (r - CTPMain.startIndex) % 3;
                CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.FREQ].Value = CTPMain.chFreq[freIdx];
                int row = (r - CTPMain.startIndex) + (int)eTestItem.RV_PER_CH0;
