...
namespace Medicare.BLE
{
    public partial class CFD : Form
    {
        ...
        {
            cfdConditionLabel.BackColor = Color.FromArgb(0, 126, 222);
            cfdConditionLabel.ForeColor = Color.White;

            cfdSpecLabel.BackColor = Color.FromArgb(0, 126, 222);
            cfdSpecLabel.ForeColor = Color.White;
        ...
        private void load()
        {
            channelTextBox.Text = CTPMain.btConfigFile["cfod"]["channel"].ToString();
            packetNumTextBox.Text = CTPMain.btConfigFile["cfod"]["npackets"].ToString();

            freqAccuracyTextBox.Text = CTPMain.btConfigFile["cfod"]["freqacc"].ToString();
            freqDriftTextBox.Text = CTPMain.btConfigFile["cfod"]["freqdrift"].ToString();
            initialFreqDriftTextBox.Text = CTPMain.btConfigFile["cfod"]["initfreqdrift"].ToString();
            maxDriftRateTextBox.Text = CTPMain.btConfigFile["cfod"]["maxdriftrate"].ToString();
        }
        public static void carrierFreqModCommands(out List<string> list)
        {
            List<string> cmdList = new List<string>();
            var channel = CTPMain.channel[(int)eBLECaseIdx.CFD];
            // 1
            cmdList.Add("CNT,0");
            cmdList.Add(string.Format("CMD,CONFigure:BT:LE:TCase:CFD:CHANnel {0}", channel));
            cmdList.Add("CMD,CONF:BT:LE:TC:CFD:DLENgth 37");
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:CFD:NPACkets {0}", CTPMain.packetNum[(int)eBLECaseIdx.CFD]));
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:CFD:LIM:DF_MAX_UP {0}", freq_accuracy));
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:CFD:LIM:F_DRIFT01_MAX_UP {0}", initial_freq_drift));
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:CFD:LIM:F_DRIFT_MAX_UP {0}", freq_drift));
            cmdList.Add(string.Format("CMD,CONF:BT:LE:TC:CFD:LIM:DRIFT_RATE_MAX_UP {0}", max_drift_rate));

            cmdList.Add("CMD,READ:BT:LE:TC:CFD?");
            cmdList.Add("CMD,READ:BT:LE:TC:CFD:VERDict?");
        ...
        public static string getCFDReport(float time)
        {
            ...
            try
            {
                report = string.Format(
                    ...
                    "       |f[n]-f[n-5]| <= {4} kHz\n" +
                    ":::Results:::\n",
                    CTPMain.packetNum[(int)eBLECaseIdx.CFD],
                    freq_accuracy,
                    initial_freq_drift,
                    freq_drift,
                    max_drift_rate);
                ...
                var channel = CTPMain.channel[(int)eBLECaseIdx.CFD].Split('-');
            ...
            {
                CTPMain.wnd.setTextPgrBar("CFD Error", Color.Red);
        ...
        private void save()
        {
            ...
            var c = s.Split('-');
            CTPMain.channel[(int)eBLECaseIdx.CFD] = c[0] + "-" + c[1] + "-" + c[2];
            CTPMain.packetNum[(int)eBLECaseIdx.CFD] = packetNumTextBox.Text;
            ...
            CTPMain.btConfigFile["cfod"]["channel"] = CTPMain.channel[(int)eBLECaseIdx.CFD];
            CTPMain.btConfigFile["cfod"]["npackets"] = CTPMain.packetNum[(int)eBLECaseIdx.CFD];
            CTPMain.btConfigFile["cfod"]["freqacc"] = freq_accuracy;
            CTPMain.btConfigFile["cfod"]["freqdrift"] = freq_drift;
            CTPMain.btConfigFile["cfod"]["initfreqdrift"] = initial_freq_drift;
            CTPMain.btConfigFile["cfod"]["maxdriftrate"] = max_drift_rate;
        ...
        private bool addRows()
        {
            ...
            int len = CTPMain.startIndex + (int)eTestItem.CFDTIME - (int)eTestItem.ftx_fn_CH0;
            ...
            for (int r = CTPMain.startIndex; r < len; r++)
            {
                ...
                int row = (r - CTPMain.startIndex) + (int)eTestItem.ftx_fn_CH0;

                if (row <= (int)eTestItem.ftx_fn_CH39)
                {
                ...
                else if ((int)eTestItem.ftx_fn_CH39 < row && row <= (int)eTestItem.f0_fn_CH39)
                {
                ...
                else if ((int)eTestItem.f0_fn_CH39 < row && row <= (int)eTestItem.f1_f0_CH39)
                {
                ...
                else if ((int)eTestItem.f1_f0_CH39 < row && row <= (int)eTestItem.fn_fn5_CH39)
                {
                    CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.MEAS].Value = drift_rate_max[freIdx];
            ...
            CTPMain.wnd.mainBLEGrid.Rows[len].Cells[(int)eCol.TST].Value =
                CTPMain.testTime[(int)eBLECaseIdx.CFD];
        ...
        public async Task<bool> runCFD(List<string> cmdList)     // carrier freq. offset + mod. char(preamble)
        {
            try
            {
               ...
               //error = ERROR.NO_ERROR;
                ...
                bool isPass = addRows();
                //bool isPass = true;
                //for (int r = (int)eTestItem.ftx_fn_CH0; r < (int)eTestItem.CFDTIME; r++)
                //{
                //    //await Task.Delay(CTPMain.waitTime);
                //    int freIdx = (r - (int)eTestItem.avgCH0) % 3;
                //    CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.FREQ].Value = CTPMain.chFreq[freIdx];

                //    if (r <= (int)eTestItem.ftx_fn_CH39)
                //    {
                //        CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.MEAS].Value = df_max[freIdx];
                //    }
                //    else if ((int)eTestItem.ftx_fn_CH39 < r && r <= (int)eTestItem.f0_fn_CH39)
                //    {
                //        CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.MEAS].Value = drift_rate_max[freIdx];
                //    }
                //    else if ((int)eTestItem.f0_fn_CH39 < r && r <= (int)eTestItem.f1_f0_CH39)
                //    {
                //        CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.MEAS].Value = f_drift01_max[freIdx];
                //    }
                //    else if ((int)eTestItem.f1_f0_CH39 < r && r <= (int)eTestItem.fn_fn5_CH39)
                //    {
                //        CTPMain.wnd.mainBLEGrid.Rows[r].Cells[(int)eCol.MEAS].Value = drift_rate_max[freIdx];
                //    }
                ...
                //CTPMain.wnd.mainBLEGrid.Rows[(int)eTestItem.CFDTIME].Cells[(int)eCol.TST].Value =
                //    CTPMain.testTime[(int)eBLECaseIdx.CFD];
